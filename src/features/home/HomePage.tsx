...
interface HomePageProps { entries?: StressEntry[]; repository?: HomeRepository; onRecord?: () => void; onReview?: () => void; onExercises?: () => void; onSettings?: () => void }

export function HomePage({ entries: suppliedEntries, repository = entriesRepository, onRecord, onReview, onExercises, onSettings }: HomePageProps) {
  ...
  return <main className="app-content home-page">
    ...
    {error && <section role="alert"><p>读取本机记录失败：{error}</p><p>你可以重试，或前往设置页导出已有数据。</p><button type="button" onClick={load}>重试</button><button type="button" onClick={onSettings}>前往设置</button></section>}
    {!loading && !error && <>
